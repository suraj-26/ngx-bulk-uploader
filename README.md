...
This folder is ready to build with `ng-packagr`. If you don't already have an Angular
workspace with `ng generate library` set up:

```bash
ng new my-workspace --no-create-application
cd my-workspace
ng generate library ngx-bulk-uploader
```

then copy this folder's `src/`, `package.json`, `ng-package.json`, and `README.md` over the
generated `projects/ngx-bulk-uploader/`, keeping your workspace's own `tsconfig.json` /
`angular.json` references intact rather than overwriting them.
...
ng build ngx-bulk-uploader

cd dist/ngx-bulk-uploader
npm login          # once, if you haven't already
npm publish
```

If `ngx-bulk-uploader` is already taken on npm, publish under a scoped name instead — set
`"name": "@yourusername/ngx-bulk-uploader"` in `package.json` and add
`"publishConfig": { "access": "public" }` (scoped packages default to private).

**Before your first publish**, it's worth pushing this to a public GitHub repo and adding
`repository` / `homepage` / `bugs` fields to `package.json` — npm shows these on the package
page, and it's the difference between someone finding just a README and finding a real,
browsable codebase (including `examples/`) to reference when something's unclear.

**To publish an update** later: bump `version` in `package.json` (semver — patch for fixes,
minor for new fields/options, major for anything that breaks an existing schema/adapter),
`ng build ngx-bulk-uploader` again, then `npm publish` from `dist/ngx-bulk-uploader` again.
